...
export interface Platform {
  /** Identifies the ledge to whoever is drawing it; the physics ignores it. */
  id?: string;
  top: number;
  left: number;
  right: number;
...
export function stepHero(
  ...
  // requestAnimationFrame's first timestamp can predate the performance.now()
  // the caller primed its clock with. A negative step would run the timers
  // backwards — the jump buffer and coyote time both "gain" time from zero
  // and fire a jump nobody pressed.
  const delta = Math.max(0, rawDelta);
  // "Screen" here is the world the hero can move in; it can be taller than
  // the viewport when the camera scrolls.
